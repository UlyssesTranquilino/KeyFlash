...
import { getFlashcardsWithFolderId } from "../../../../../../utils/flashcard/flashcard";
...
const Folder = () => {
  ...
  const { slug } = useParams(); // "123-biology-basics"
  const parts = slug?.toString().split("-") || [];
  const folderId = parts[0]; // "123"
  const [topic, setTopic] = useState(parts.slice(1).join(" "));
  ...
  useEffect(() => {
    const fetchFolder = async () => {
      ...
      if (folder) {
        setTopic(folder.name); // override topic from DB
      }
    };
  ...
  useEffect(() => {
    const fetchFlashcards = async () => {
      const data = await getFlashcardsWithFolderId(folderId);
  ...
  const handleSortFlashcard = (value: string) => {
    ...
    const sortedData = [...filteredFlashcards];
    ...
    setFilteredFlashcards(sortedData);
  };
          ...
          {flashcards.length > 0 ? (
            filteredFlashcards.length > 0 ? (
              ...
              <div className="text-gray-400 flex flex-col items-center justify-center mt-14 sm:mt-32 lg:mt-42 gap-3 md:gap-5">
                ...
                <h2 className="md:text-lg"> No Flashcards found</h2>
              </div>
            )
          ) : (
            <div className="mt-12">
