...
export default function HomePage() {
  ...
  const handleSearchText = (searchTerm: string) => {
    ...
    const filteredData = codes.filter((card) =>
      card.title.toLowerCase().includes(searchTerm.toLowerCase())
    );
    setFilteredCodes(filteredData);
  };

  const handleSortCode = (value: string) => {
    setSortMethod(value);

    const sortedData = [...filteredCodes];
      ...
      <div className="mt-12">
        ...
        <div className="mb-5 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
          ...
          <div className="self-end">
            <Select
              value={sortMethod}
              onValueChange={(value) => handleSortCode(value)}
            >
          ...
          {codes.length > 0 ? (
            filteredCodes.length > 0 ? (
              ...
              <div className="text-gray-400 flex flex-col items-center justify-center mt-14 sm:mt-32 lg:mt-42 gap-3 md:gap-5">
                ...
                <h2 className="md:text-lg"> No Codes found</h2>
              </div>
            )
          ) : (
            <div className="mt-12">
